...
- ✅ **v0.15.0** — Productivity Workspace: pinned notes, recent notes, workspace tabs (lightweight strip over the single-active-note editor), layout presets, a Dashboard (stats/quick-actions/presets/pinned/recent), and global search across notes/equations/citations/attachments (calculator blocks now indexed into note search too).
- ✅ **v0.16.0** — Data Protection: automatic per-note version history (snapshot on switch-away, preview/restore any version), one-click vault backup/restore (raw checkpointed `.db` file, no new dependency), and named workspace snapshots (save/restore a set of open tabs).
- ⏭️ **Next: v0.17.0** — Laboratory Workspace. Deferred items to fold in later: the visual equation dependency **graph** (from v0.3.0), equation→calculator integration (from v0.4.0), in-app audio/video players for attachments (from v0.6.0), a PDF24 launcher for merge/compress/split/OCR (from v0.7.0, deliberately skipped), and formula-sheet PDF/print export (from v0.10.0 — `PrintLayer` is currently note-shaped only).
