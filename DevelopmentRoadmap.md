...
- ⏭️ **Next: v0.16.0** — Data Protection. Deferred items to fold in later: the visual equation dependency **graph** (from v0.3.0), equation→calculator integration (from v0.4.0), in-app audio/video players for attachments (from v0.6.0), a PDF24 launcher for merge/compress/split/OCR (from v0.7.0, deliberately skipped), and formula-sheet PDF/print export (from v0.10.0 — `PrintLayer` is currently note-shaped only).
